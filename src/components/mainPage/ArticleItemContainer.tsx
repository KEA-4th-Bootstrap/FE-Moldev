...
const ArticleItemContainer = ({ item }: { item: ArticleListItemType }) => {
  return (
    <div className="grow h-[300px] flex flex-col items-center justify-center rounded-card gap-y-10 bg-white hover:bg-main/10 cursor-pointer">
      <div className="shrink-0 h-[120px] w-full overflow-hidden">
        <img className="w-full h-full object-cover rounded-t-card" src={item.img} alt="article" />
      </div>
      <div className="w-full grow flex flex-col justify-between items-start px-13">
        <div className="w-full flex flex-col justify-start items-start gap-y-6">
          <div className="w-full text-16 font-bold line-clamp-1">{item.title}</div>
          <div className="w-full text-14 font-normal line-clamp-3">{item.content}</div>
        </div>
