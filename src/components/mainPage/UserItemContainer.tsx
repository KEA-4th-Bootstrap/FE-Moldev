...
const UserItemContainer = ({ item }: { item: UserListItemType }) => {
  return (
    <div className="grow flex flex-col items-center justify-center rounded-card gap-y-10 py-20 bg-white hover:bg-main/10 cursor-pointer">
      <div className="w-full flex items-center justify-start px-14 gap-x-12">
        <img className="w-[52px] h-[52px] rounded-full" src={item.userImg} alt="user" />
      ...
      <div className="w-full px-16 flex flex-col items-center justify-center gap-y-3">
        {item.articles.map((article) => (
          <div className="w-full flex items-center justify-between" key={article.id}>
            <div className="text-14 font-semibold truncate">{article.title}</div>
