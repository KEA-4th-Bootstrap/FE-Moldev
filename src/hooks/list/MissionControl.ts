...
export const MissionControl = (
  ...
  const firstCount: number = first || Math.floor(Math.random() * Math.floor(length / 2) + 2);
  ...
  const calculateItemDimension = (m: number): ItemDimensions[] => {
    ...
    for (let i = 0; i < m; i++) {
      const currentWidth =
        Math.floor(Math.random() * (currentMaxItemWidth - currentMinItemWidth)) +
        currentMinItemWidth;
      const currentHeight = Math.min(Math.floor(currentWidth / 1.3), currentMaxItemHeight);
